...
Tjiftjaf is an MQTT 3.1.1 client written in Rust.

Below, you find an example of how `tjiftjaf` integrates with the
...
1. The `Client` interacting with a network socket.
...
The `MqttBinding` is the heart of `tjiftjaf`. The `MqttBinding` is implemented according to sans-IO.
...
The binding includes two state machines.
One state machine keeps track of the connection status. An MQTT client is only allowed to
send MQTT packets _after_ it receives a positive `ConnAck` packet from the server.

The other state machine decodes bytes as `Packet`s. It tracks how many more bytes
are required to construct a `Packet`. `MqttBinding.get_input_buffer()` hands out buffers
with a length that's dictated by that state machine.
